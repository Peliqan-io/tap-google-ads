#!/usr/bin/env python3
"""
Baseline capture for tap-google-ads — run once on Python 3.9.
...
Required env vars:
    ...
    TAP_GOOGLE_ADS_MANAGER_ACCOUNT_ID    (10-digit MCC, no dashes)
    TAP_GOOGLE_ADS_ACCOUNT_IDS           (comma-separated 10-digit customer IDs)
    TAP_GOOGLE_ADS_INCLUDE_STREAMS       (default "campaign,customer")
    TAP_GOOGLE_ADS_START_DATE            (default 2026-01-01T00:00:00Z)
"""
import json
import os
import subprocess
...
# manager_account_id is optional — empty/unset means the OAuth user accesses
# the customer directly (no manager login context).
...
def main():
    ...
    include = os.getenv("TAP_GOOGLE_ADS_INCLUDE_STREAMS", "campaign,customer")
